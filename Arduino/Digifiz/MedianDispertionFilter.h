...
#ifndef _MedianDispertionFilter_h
#define _MedianDispertionFilter_h
...
template <typename T>
class MedianDispertionFilter
{

typedef T (MedianDispertionFilter::*Action)(T);

public:
	MedianDispertionFilter<T>(const size_t windowSize);
	T AddValue(T item);
	T GetFiltered() const;
  T GetDispertion() const;
  T GetMean() const;
private:
	struct node
	{
...
template<typename T>
MedianDispertionFilter<T>::MedianDispertionFilter(const size_t windowSize)
{
	...
	if(_windowSize == 3)
		addValue = &MedianDispertionFilter::addValue3;
	else
		addValue = &MedianDispertionFilter::addValueN;
}

template<typename T>
T MedianDispertionFilter<T>::AddValue(T value)
{
	return (*this.*addValue)(value);
}

template<typename T>
T MedianDispertionFilter<T>::GetFiltered() const
{
	return _lastFiltered;
}

template<typename T>
T MedianDispertionFilter<T>::addValueN(T value)
{
...
template<typename T>
T MedianDispertionFilter<T>::addValue3(T value)
{
...
template<typename T>
T MedianDispertionFilter<T>::median3(T a, T b, T c)
{
	if ((a <= b) && (a <= c))
		return (b <= c) ? b : c;
	if ((b <= a) && (b <= c))
